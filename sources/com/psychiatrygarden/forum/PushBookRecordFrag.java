...
public class PushBookRecordFrag extends Fragment implements OnRefreshLoadMoreListener {
    ...
    private void loadData() {
        ...
        YJYHttpUtils.post(getActivity(), str, ajaxParams, new AjaxCallBack<String>() { // from class: com.psychiatrygarden.forum.PushBookRecordFrag.2
            @Override // net.tsz.afinal.http.AjaxCallBack
            public void onFailure(Throwable t2, int errorNo, String strMsg) {
                super.onFailure(t2, errorNo, strMsg);
                PushBookRecordFrag.this.mRefresh.finishRefresh();
                if (PushBookRecordFrag.this.page == 1) {
                    PushBookRecordFrag.this.emptyView.setLoadFileResUi(PushBookRecordFrag.this.getActivity());
                }
            }
            ...
            public void onSuccess(String s2) {
                ...
                PushBookRecordFrag.this.emptyView.showEmptyView();
                PushBookRecordFrag.this.mRefresh.finishRefresh();
                PushBookRecordFrag.this.isLoadData = true;
                try {
                    ...
                    if (bookShelfBean.getCode().equals("200")) {
                        if (bookShelfBean.getData() == null || bookShelfBean.getData().isEmpty()) {
                            if (PushBookRecordFrag.this.page == 1) {
                                PushBookRecordFrag.this.mAdapter.setList(new ArrayList());
                                PushBookRecordFrag.this.mAdapter.setEmptyView(PushBookRecordFrag.this.emptyView);
                                return;
                            }
                            return;
                        }
                        ...
                        for (int i2 = 0; i2 < bookShelfBean.getData().size(); i2++) {
                            ...
                            if (PushBookRecordFrag.this.mChooseBookIds.contains(pushBookData.getId())) {
                                pushBookData.setSelected(true);
                            }
                            arrayList.add(pushBookData);
                        }
                        if (PushBookRecordFrag.this.page == 1) {
                            PushBookRecordFrag.this.mAdapter.setSearchContent(PushBookRecordFrag.this.mSearchKey);
                            PushBookRecordFrag.this.mAdapter.setList(arrayList);
                            if (bookShelfBean.getData().size() < 10) {
                                PushBookRecordFrag.this.mRefresh.finishLoadMoreWithNoMoreData();
                                return;
                            }
                            return;
                        }
                        ...
                        PushBookRecordFrag.this.mAdapter.addData((Collection) arrayList);
                        if (bookShelfBean.getData().size() < 10) {
                            PushBookRecordFrag.this.mRefresh.finishLoadMoreWithNoMoreData();
                        } else {
                            PushBookRecordFrag.this.mRefresh.finishLoadMore();
                        }
                    }
                } catch (Exception e2) {
                    e2.printStackTrace();
    ...
    public static PushBookRecordFrag newInstance(boolean isShelf, List<String> bookIdList) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("isShelf", isShelf);
        bundle.putSerializable("bookIdList", (Serializable) bookIdList);
        PushBookRecordFrag pushBookRecordFrag = new PushBookRecordFrag();
        pushBookRecordFrag.setArguments(bundle);
        return pushBookRecordFrag;
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        customEmptyView.setOnReloadData(new View.OnClickListener() { // from class: com.psychiatrygarden.forum.n0
            @Override // android.view.View.OnClickListener
            public final void onClick(View view2) {
                this.f15385c.lambda$onViewCreated$0(view2);
            }
        });
        this.mAdapter.setEmptyView(this.emptyView);
        this.mRecycler.setAdapter(this.mAdapter);
        loadData();
        this.mAdapter.setOnItemChoosedLisenter(new PushBookAdp.OnChooseItemLisenter() { // from class: com.psychiatrygarden.forum.PushBookRecordFrag.1
            ...
            public void onItemChoosed(int pos, PushBookData item) {
                if (PushBookRecordFrag.this.mChooseBookIds.size() < 10) {
                    item.setSelected(!item.isSelected());
                    PushBookRecordFrag.this.mAdapter.notifyItemChanged(pos, 1);
                    Intent intent = new Intent("updateBottomUi");
                    intent.putExtra("book", item);
                    LocalBroadcastManager.getInstance(PushBookRecordFrag.this.requireContext()).sendBroadcast(intent);
                    return;
                }
                if (!item.isSelected()) {
                    ToastUtil.shortToast(PushBookRecordFrag.this.getContext(), "最多只能添加10本书籍");
                    return;
                }
                item.setSelected(!item.isSelected());
                PushBookRecordFrag.this.mAdapter.notifyItemChanged(pos, 1);
                ...
                LocalBroadcastManager.getInstance(PushBookRecordFrag.this.requireContext()).sendBroadcast(intent2);
            }
        });
    }
