...
public abstract class JSON implements JSONStreamAware, JSONAware {
    ...
    private static void config(Properties properties) {
        ...
        if (a.f27523u.equals(properties.getProperty("parser.features.NonStringKeyAsString"))) {
            DEFAULT_PARSER_FEATURE |= Feature.NonStringKeyAsString.getMask();
        }
        if (a.f27523u.equals(properties.getProperty("parser.features.ErrorOnEnumNotMatch")) || a.f27523u.equals(properties.getProperty("fastjson.parser.features.ErrorOnEnumNotMatch"))) {
            DEFAULT_PARSER_FEATURE |= Feature.ErrorOnEnumNotMatch.getMask();
        }
        if (a.f27524v.equals(properties.getProperty("fastjson.asmEnable"))) {
    ...
    public static Object parse(byte[] bArr, int i2, int i3, CharsetDecoder charsetDecoder, Feature... featureArr) {
        ...
        return parse(bArr, i2, i3, charsetDecoder, iConfig);
    }
    ...
    public static Object parse(byte[] bArr, int i2, int i3, CharsetDecoder charsetDecoder, int i4) throws CharacterCodingException {
        charsetDecoder.reset();
        char[] cArrAllocateChars = allocateChars((int) (i3 * charsetDecoder.maxCharsPerByte()));
        ...
        DefaultJSONParser defaultJSONParser = new DefaultJSONParser(cArrAllocateChars, charBufferWrap.position(), ParserConfig.getGlobalInstance(), i4);
        Object obj = defaultJSONParser.parse();
        defaultJSONParser.handleResovleTask(obj);
        defaultJSONParser.close();
        return obj;
    ...
    public static <T> T parseObject(byte[] bArr, int i2, int i3, Charset charset, Type type, ParserConfig parserConfig, ParseProcess parseProcess, int i4, Feature... featureArr) {
        ...
        if (charset == IOUtils.UTF8) {
            char[] cArrAllocateChars = allocateChars(bArr.length);
            int iDecodeUTF8 = IOUtils.decodeUTF8(bArr, i2, i3, cArrAllocateChars);
            if (iDecodeUTF8 < 0) {
                return null;
            }
    ...
    public static <T> T parseObject(byte[] bArr, int i2, int i3, CharsetDecoder charsetDecoder, Type type, Feature... featureArr) throws CharacterCodingException {
        charsetDecoder.reset();
        char[] cArrAllocateChars = allocateChars((int) (i3 * charsetDecoder.maxCharsPerByte()));
        ByteBuffer byteBufferWrap = ByteBuffer.wrap(bArr, i2, i3);
        CharBuffer charBufferWrap = CharBuffer.wrap(cArrAllocateChars);
    ...
    public static <T> T parseObject(char[] cArr, int i2, Type type, Feature... featureArr) {
        ...
        DefaultJSONParser defaultJSONParser = new DefaultJSONParser(cArr, i2, ParserConfig.getGlobalInstance(), iConfig);
        T t2 = (T) defaultJSONParser.parseObject(type);
        defaultJSONParser.handleResovleTask(t2);
        defaultJSONParser.close();
        return t2;
    ...
    public static <T> T parseObject(InputStream inputStream, Charset charset, Type type, ParserConfig parserConfig, ParseProcess parseProcess, int i2, Feature... featureArr) throws IOException {
        ...
        int i3 = 0;
        while (true) {
            int i4 = inputStream.read(bArrAllocateBytes, i3, bArrAllocateBytes.length - i3);
            if (i4 == -1) {
                return (T) parseObject(bArrAllocateBytes, 0, i3, charset2, type, parserConfig, parseProcess, i2, featureArr);
            }
            i3 += i4;
            if (i3 == bArrAllocateBytes.length) {
