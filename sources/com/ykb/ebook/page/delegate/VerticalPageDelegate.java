...
public abstract class VerticalPageDelegate extends PageDelegate {
    ...
    public VerticalPageDelegate(@NotNull ReadView readView) {
        super(readView);
    ...
    private final void onScroll(MotionEvent event) {
        ...
        if (getIsMoved()) {
            if (getMDirection() != PageDirection.NEXT ? y2 < getLastY() : y2 > getLastY()) {
                z2 = true;
            }
            setCancel(z2);
            setRunning(true);
    ...
    public void nextPageByAnim(int animationSpeed) throws SecurityException, NumberFormatException {
        ...
        if (hasNext() || !ReadConfig.INSTANCE.getLastPage()) {
            setDirection(PageDirection.NEXT);
            getReadView().setStartPoint(getStartX() > ((float) (getViewWidth() / 2)) ? getViewWidth() * 0.9f : 1.0f, getViewHeight() * 0.9f, false);
    ...
    @Override // com.ykb.ebook.page.delegate.PageDelegate
    public void onTouch(@NotNull MotionEvent event) throws SecurityException, NumberFormatException {
