...
public class W3CEvent {
    ...
    public static W3CEvent createFromJSON(String str) throws JSONException {
        ...
        try {
            ...
            if (!jSONObject.isNull("recurrence")) {
                ...
                if (!jSONObject2.isNull(am.aT)) {
                    w3CEvent.getRecurrence().setInterval(jSONObject2.getInt(am.aT));
                }
                if (!jSONObject2.isNull(ClientCookie.EXPIRES_ATTR)) {
                    w3CEvent.getRecurrence().setExpires(jSONObject2.getString(ClientCookie.EXPIRES_ATTR));
                }
                if (!jSONObject2.isNull("exceptionDates")) {
                    ...
                    for (int i2 = 0; i2 < length; i2++) {
                        w3CEvent.getRecurrence().getExceptionDates()[i2] = jSONArray.getString(i2);
                    }
                }
                if (!jSONObject2.isNull("daysInWeek")) {
                    ...
                    for (int i3 = 0; i3 < length2; i3++) {
                        w3CEvent.getRecurrence().getDaysInWeek()[i3] = jSONArray2.getInt(i3);
                    }
                }
                if (!jSONObject2.isNull("daysInMonth")) {
                    ...
                    for (int i4 = 0; i4 < length3; i4++) {
                        w3CEvent.getRecurrence().getDaysInMonth()[i4] = jSONArray3.getInt(i4);
                    }
                }
                if (!jSONObject2.isNull("daysInYear")) {
                    ...
                    for (int i5 = 0; i5 < length4; i5++) {
                        w3CEvent.getRecurrence().getDaysInYear()[i5] = jSONArray4.getInt(i5);
                    }
                }
                if (!jSONObject2.isNull("weeksInMonth")) {
                    ...
                    for (int i6 = 0; i6 < length5; i6++) {
                        w3CEvent.getRecurrence().getWeeksInMonth()[i6] = jSONArray5.getInt(i6);
                    }
                }
                if (!jSONObject2.isNull("monthsInYear")) {
                    ...
                    for (int i7 = 0; i7 < length6; i7++) {
                        w3CEvent.getRecurrence().getMonthsInYear()[i7] = jSONArray6.getInt(i7);
                    }
                }
            }
    ...
    public Intent getInsertIntent() {
        boolean z2 = !useMIME;
        Intent data = z2 ? new Intent("android.intent.action.EDIT").setData(CalendarContract.Events.CONTENT_URI) : new Intent("android.intent.action.EDIT").setType("vnd.android.cursor.item/event");
        if (!StringUtil.isEmpty(getDescription())) {
            if (z2) {
                data.putExtra("title", getDescription());
            } else {
                data.putExtra("title", getDescription());
            }
        }
        ...
        if (!StringUtil.isEmpty(getStart())) {
            ...
            if (jMillisFromDateString > 0) {
                if (z2) {
                    data.putExtra("beginTime", jMillisFromDateString);
                } else {
                    data.putExtra("beginTime", jMillisFromDateString);
                }
            }
        ...
        if (!StringUtil.isEmpty(getEnd())) {
            ...
            if (jMillisFromDateString2 > 0) {
                if (z2) {
                    data.putExtra("endTime", jMillisFromDateString2);
                } else {
                    data.putExtra("endTime", jMillisFromDateString2);
                }
            }
        }
        if (!StringUtil.isEmpty(getStatus()) && z2) {
            data.putExtra("eventStatus", getStatus());
        }
        if (!StringUtil.isEmpty(getTransparency()) && z2) {
        ...
        if (!StringUtil.isEmpty(getReminder())) {
            ...
            if (jMillisFromDateString3 < 0) {
                if (z2) {
                    data.putExtra("minutes", Math.abs(jMillisFromDateString3 / 60000));
                }
            } else if (!StringUtil.isEmpty(getStart()) && z2) {
                long jMillisFromDateString4 = millisFromDateString(getStart());
                if (jMillisFromDateString4 > 0) {
                    data.putExtra("minutes", Math.abs((jMillisFromDateString4 - jMillisFromDateString3) / 60000));
                }
            }
        }
        String str = "";
        ...
        if (getRecurrence() != null) {
            ...
            if (W3C_WEEKLY.equals(str) && getRecurrence().getDaysInWeek() != null && getRecurrence().getDaysInWeek().length > 0) {
                sb.append("BYDAY=");
                for (int i2 : getRecurrence().getDaysInWeek()) {
                    switch (i2) {
                        case 0:
                            sb.append("SU,");
                            break;
            ...
            if (W3C_MONTHLY.equals(str) && getRecurrence().getDaysInMonth() != null && getRecurrence().getDaysInMonth().length > 0) {
                ...
                for (int i3 : getRecurrence().getDaysInMonth()) {
                    sb.append(i3);
                    sb.append(",");
                }
                sb.setCharAt(sb.length() - 1, ';');
            ...
            if (W3C_YEARLY.equals(str) && getRecurrence().getDaysInYear() != null && getRecurrence().getDaysInYear().length > 0) {
                sb.append("BYYEARDAY=");
                for (int i4 : getRecurrence().getDaysInYear()) {
                    sb.append(i4);
                    sb.append(",");
                }
                sb.setCharAt(sb.length() - 1, ';');
            ...
            if (W3C_YEARLY.equals(str) && getRecurrence().getMonthsInYear() != null && getRecurrence().getMonthsInYear().length > 0) {
                sb.append("BYMONTH=");
                for (int i5 : getRecurrence().getMonthsInYear()) {
                    sb.append(i5);
                    sb.append(",");
                }
                sb.setCharAt(sb.length() - 1, ';');
            ...
            if (W3C_MONTHLY.equals(str) && getRecurrence().getWeeksInMonth() != null && getRecurrence().getWeeksInMonth().length > 0) {
                sb.append("BYWEEKNO=");
                for (int i6 : getRecurrence().getWeeksInMonth()) {
                    sb.append(i6);
                    sb.append(",");
                }
                sb.setCharAt(sb.length() - 1, ';');
