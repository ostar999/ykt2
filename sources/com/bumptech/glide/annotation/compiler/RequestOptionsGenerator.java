...
final class RequestOptionsGenerator {
    ...
    private StringBuilder createNewOptionAndCall(boolean z2, MethodSpec.Builder builder, String str, List<ParameterSpec> list) {
        StringBuilder sb = new StringBuilder(str);
        if (!list.isEmpty()) {
            builder.addParameters(list);
            for (ParameterSpec parameterSpec : list) {
                sb.append(parameterSpec.name);
                if (z2 && isAndroidContext(parameterSpec)) {
    ...
    public MethodAndStaticVar generateStaticMethodEquivalentForExtensionMethod(ExecutableElement executableElement) {
        ...
        if (Strings.isNullOrEmpty(staticMethodName)) {
            if (string.startsWith("dont")) {
                staticMethodName = SVGParser.XML_STYLESHEET_ATTR_ALTERNATE_NO + string.replace("dont", "");
            } else {
        ...
        StringBuilder sbCreateNewOptionAndCall = createNewOptionAndCall(zMemoizeStaticMethodFromAnnotation, builderReturns, "new $T().$L(", ProcessorUtil.getParameters((List<? extends VariableElement>) parameters.subList(1, parameters.size())));
        if (zMemoizeStaticMethodFromAnnotation) {
            StringBuilder sb = new StringBuilder();
            sb.append(staticMethodName);
            int i2 = this.nextFieldId;
            this.nextFieldId = i2 + 1;
            sb.append(i2);
        ...
        builderReturns.addAnnotation(ProcessorUtil.checkResult());
    ...
    private MethodAndStaticVar generateStaticMethodEquivalentForRequestOptionsStaticMethod(ExecutableElement executableElement) {
        ...
        StringBuilder sbCreateNewOptionAndCall = createNewOptionAndCall(zMemoizeStaticMethodFromArguments, builderReturns, "new $T().$N(", ProcessorUtil.getParameters(executableElement));
        ...
        builderReturns.addAnnotation(ProcessorUtil.checkResult()).addAnnotation(ProcessorUtil.nonNull());
        return new MethodAndStaticVar(builderReturns.build(), fieldSpecBuild);
    }
    ...
    private static String getInstanceMethodNameFromStaticMethodName(String str) {
        ...
        if (str.equals(Constants.PARAMS_CONSTANTS.PARAMS_TOPIC_OPTION)) {
            return "set";
        }
    ...
    public TypeSpec generate(String str, Set<String> set) {
        ...
        for (MethodAndStaticVar methodAndStaticVar2 : arrayList2) {
            ...
            if (methodSpec2 != null) {
                builderSuperclass.addMethod(methodSpec2);
            }
            FieldSpec fieldSpec = methodAndStaticVar2.staticField;
            if (fieldSpec != null) {
                builderSuperclass.addField(fieldSpec);
            }
        }
