...
final class GlideExtensionValidator {
    ...
    private static List<String> getComparableParameterNames(ExecutableElement executableElement, boolean z2) {
    ...
    private static String getQualifiedMethodName(ExecutableElement executableElement) {
        return getEnclosingClassName(executableElement) + DictionaryFactory.SHARP + getName(executableElement);
    }
    ...
    private void validateAnnotatedNonNull(ExecutableElement executableElement) {
        ...
        if (z2) {
            this.processingEnvironment.getMessager().printMessage(Diagnostic.Kind.WARNING, getQualifiedMethodName(executableElement) + " is missing the " + ProcessorUtil.nonNull().reflectionName() + " annotation, please add it to ensure that your extension methods are always returning non-null values");
        }
    }
