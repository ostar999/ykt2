...
final class RequestBuilderGenerator {
    ...
    private static final AnnotationSpec NON_NULL = AnnotationSpec.builder(ProcessorUtil.nonNull()).build();
    private static final AnnotationSpec CHECK_RESULT = AnnotationSpec.builder(ProcessorUtil.checkResult()).build();
    ...
    private List<MethodSpec> generateConstructors() {
        ...
        return ImmutableList.of(MethodSpec.constructorBuilder().addParameter(ParameterSpec.builder(parameterizedTypeName, "transcodeClass", new Modifier[0]).addAnnotation(ProcessorUtil.nonNull()).build()).addParameter(ParameterSpec.builder(ParameterizedTypeName.get(ClassName.get(this.requestBuilderType), WildcardTypeName.subtypeOf(Object.class)), "other", new Modifier[0]).addAnnotation(ProcessorUtil.nonNull()).build()).addStatement("super($N, $N)", "transcodeClass", "other").build(), MethodSpec.constructorBuilder().addParameter(ParameterSpec.builder(ClassName.get("com.bumptech.glide", "Glide", new String[0]), "glide", new Modifier[0]).addAnnotation(ProcessorUtil.nonNull()).build()).addParameter(ParameterSpec.builder(ClassName.get("com.bumptech.glide", "RequestManager", new String[0]), "requestManager", new Modifier[0]).addAnnotation(ProcessorUtil.nonNull()).build()).addParameter(ParameterSpec.builder(parameterizedTypeName, "transcodeClass", new Modifier[0]).addAnnotation(ProcessorUtil.nonNull()).build()).addParameter(ParameterSpec.builder(ClassName.get("android.content", "Context", new String[0]), d.R, new Modifier[0]).addAnnotation(ProcessorUtil.nonNull()).build()).addStatement("super($N, $N ,$N, $N)", "glide", "requestManager", "transcodeClass", d.R).build());
    }

    private MethodSpec generateDownloadOnlyRequestMethod() {
        return MethodSpec.methodBuilder("getDownloadOnlyRequest").addAnnotation(Override.class).addAnnotation(CHECK_RESULT).addAnnotation(NON_NULL).returns(ParameterizedTypeName.get(this.generatedRequestBuilderClassName, ClassName.get((Class<?>) File.class))).addModifiers(Modifier.PROTECTED).addStatement("return new $T<>($T.class, $N).apply($N)", this.generatedRequestBuilderClassName, File.class, "this", "DOWNLOAD_ONLY_OPTIONS").build();
    }
    ...
    private List<MethodSpec> generateGeneratedRequestOptionsEquivalents(final List<MethodSpec> list, TypeSpec typeSpec) {
        return typeSpec == null ? Collections.emptyList() : FluentIterable.from(typeSpec.methodSpecs).filter(new Predicate<MethodSpec>() { // from class: com.bumptech.glide.annotation.compiler.RequestBuilderGenerator.2
            @Override // com.bumptech.glide.repackaged.com.google.common.base.Predicate
    ...
    public TypeSpec generate(String str, Set<String> set, TypeSpec typeSpec) {
        if (typeSpec != null) {
