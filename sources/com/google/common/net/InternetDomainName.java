...
public final class InternetDomainName {
    ...
    public InternetDomainName(String str) {
        ...
        lowerCase = lowerCase.endsWith(StrPool.DOT) ? lowerCase.substring(0, lowerCase.length() - 1) : lowerCase;
    ...
    private static boolean validatePart(String str, boolean z2) {
        if (str.length() >= 1 && str.length() <= 63) {
            ...
            if (!charMatcher.matches(str.charAt(0)) && !charMatcher.matches(str.charAt(str.length() - 1))) {
                return (z2 && CharMatcher.digit().matches(str.charAt(0))) ? false : true;
            }
        }
        return false;
    }
    ...
    public InternetDomainName child(String str) {
        return from(((String) Preconditions.checkNotNull(str)) + StrPool.DOT + this.name);
    }
