...
class DefaultDrmSession implements DrmSession {
    ...
    public DefaultDrmSession(UUID uuid, ExoMediaDrm exoMediaDrm, ProvisioningManager provisioningManager, ReferenceCountListener referenceCountListener, @Nullable List<DrmInitData.SchemeData> list, int i2, boolean z2, boolean z3, @Nullable byte[] bArr, HashMap<String, String> map, MediaDrmCallback mediaDrmCallback, Looper looper, LoadErrorHandlingPolicy loadErrorHandlingPolicy) {
        if (i2 == 1 || i2 == 3) {
    ...
    @RequiresNonNull({PLVLinkMicManager.SESSION_ID})
    ...
    @EnsuresNonNullIf(expression = {PLVLinkMicManager.SESSION_ID}, result = true)
    ...
    private void onError(final Exception exc, int i2) {
        this.lastException = new DrmSession.DrmSessionException(exc, DrmUtil.getErrorCodeForMediaDrmException(exc, i2));
    ...
    @RequiresNonNull({PLVLinkMicManager.SESSION_ID, "offlineLicenseKeySetId"})
