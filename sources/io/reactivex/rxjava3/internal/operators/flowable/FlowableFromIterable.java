...
public final class FlowableFromIterable<T> extends Flowable<T> {
    ...
    public static <T> void subscribe(Subscriber<? super T> s2, Iterator<? extends T> it) {
        try {
            if (!it.hasNext()) {
                ...
                s2.onSubscribe(new IteratorConditionalSubscription((ConditionalSubscriber) s2, it));
            } else {
                s2.onSubscribe(new IteratorSubscription(s2, it));
            }
        } catch (Throwable th) {
            Exceptions.throwIfFatal(th);
    ...
    public void subscribeActual(Subscriber<? super T> s2) {
        try {
            subscribe(s2, this.source.iterator());
        } catch (Throwable th) {
            Exceptions.throwIfFatal(th);
            EmptySubscription.error(th, s2);
        }
