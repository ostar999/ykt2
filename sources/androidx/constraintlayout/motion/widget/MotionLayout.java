...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public class DevModeDraw {
        ...
        private void drawPathCartesianTicks(Canvas canvas, float x2, float y2) {
            ...
            String str = "" + (((int) (((fMin2 * 100.0f) / Math.abs(f4 - f2)) + 0.5d)) / 100.0f);
            ...
            String str2 = "" + (((int) (((fMax2 * 100.0f) / Math.abs(f5 - f3)) + 0.5d)) / 100.0f);
        ...
        private void drawPathScreenTicks(Canvas canvas, float x2, float y2, int viewWidth, int viewHeight) {
            String str = "" + (((int) ((((x2 - (viewWidth / 2)) * 100.0f) / (MotionLayout.this.getWidth() - viewWidth)) + 0.5d)) / 100.0f);
            getTextBounds(str, this.mTextPaint);
            ...
            String str2 = "" + (((int) ((((y2 - (viewHeight / 2)) * 100.0f) / (MotionLayout.this.getHeight() - viewHeight)) + 0.5d)) / 100.0f);
            getTextBounds(str2, this.mTextPaint);
        ...
        private void drawTranslation(Canvas canvas, float x12, float y12, float x2, float y2) {
            canvas.drawRect(x12, y12, x2, y2, this.mPaintGraph);
            canvas.drawLine(x12, y12, x2, y2, this.mPaintGraph);
        }
    ...
    public class Model {
        ...
        private void debugLayout(String title, ConstraintWidgetContainer c3) {
            String str = title + " " + Debug.getName((View) c3.getCompanionWidget());
            Log.v(MotionLayout.TAG, str + "  ========= " + c3);
            int size = c3.getChildren().size();
            for (int i2 = 0; i2 < size; i2++) {
                String str2 = str + StrPool.BRACKET_START + i2 + "] ";
                ConstraintWidget constraintWidget = c3.getChildren().get(i2);
                StringBuilder sb = new StringBuilder();
                sb.append("");
                ConstraintAnchor constraintAnchor = constraintWidget.mTop.mTarget;
                String str3 = StrPool.UNDERLINE;
                sb.append(constraintAnchor != null ? ExifInterface.GPS_DIRECTION_TRUE : StrPool.UNDERLINE);
                String string = sb.toString();
                StringBuilder sb2 = new StringBuilder();
                sb2.append(string);
                sb2.append(constraintWidget.mBottom.mTarget != null ? "B" : StrPool.UNDERLINE);
                String string2 = sb2.toString();
                StringBuilder sb3 = new StringBuilder();
                sb3.append(string2);
                sb3.append(constraintWidget.mLeft.mTarget != null ? "L" : StrPool.UNDERLINE);
    ...
    private boolean handlesTouchEvent(float x2, float y2, View view, MotionEvent event) {
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int childCount = viewGroup.getChildCount() - 1; childCount >= 0; childCount--) {
                if (handlesTouchEvent((r3.getLeft() + x2) - view.getScrollX(), (r3.getTop() + y2) - view.getScrollY(), viewGroup.getChildAt(childCount), event)) {
                    z2 = true;
                    break;
                }
            }
        ...
        if (staggered != 0.0f) {
            ...
            if (!z2) {
                while (i2 < childCount) {
                    ...
                    motionController8.mStaggerOffset = fAbs - (((f3 - fMin2) * fAbs) / (fMax2 - fMin2));
                    i2++;
                }
                return;
            }
    ...
    */
    ...
    public static class MyTracker implements MotionTracker {
        ...
        private static MyTracker f1787me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f1787me.tracker = VelocityTracker.obtain();
            return f1787me;
        }
