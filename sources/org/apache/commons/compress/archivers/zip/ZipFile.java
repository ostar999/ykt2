...
public class ZipFile implements Closeable {
    ...
    private void resolveLocalFileHeaderData(Map<ZipArchiveEntry, NameAndComment> map) throws IOException, RuntimeException {
        ...
        while (it.hasNext()) {
            ...
            int i2 = value;
            while (i2 > 0) {
                int iSkipBytes = this.archive.skipBytes(i2);
                if (iSkipBytes <= 0) {
    ...
    private void setSizesAndOffsetFromZip64Extra(ZipArchiveEntry zipArchiveEntry, OffsetEntry offsetEntry, int i2) throws IOException {
        ...
        if (zip64ExtendedInformationExtraField != null) {
            ...
            boolean z4 = offsetEntry.headerOffset == InternalZipConstants.ZIP_64_LIMIT;
    ...
    private void skipBytes(int i2) throws IOException {
        int i3 = 0;
        while (i3 < i2) {
            int iSkipBytes = this.archive.skipBytes(i2 - i3);
            if (iSkipBytes <= 0) {
    ...
    private boolean tryToLocateSignature(long j2, long j3, byte[] bArr) throws IOException {
        long length = this.archive.length() - j2;
        long jMax = Math.max(0L, this.archive.length() - j3);
        boolean z2 = false;
        if (length >= 0) {
            while (true) {
                if (length < jMax) {
    ...
    public InputStream getInputStream(ZipArchiveEntry zipArchiveEntry) throws IOException {
        ...
        int i2 = AnonymousClass3.$SwitchMap$org$apache$commons$compress$archivers$zip$ZipMethod[ZipMethod.getMethodByCode(zipArchiveEntry.getMethod()).ordinal()];
        if (i2 == 1) {
    ...
    public ZipFile(File file, String str, boolean z2) throws IOException {
        ...
        this.OFFSET_COMPARATOR = new Comparator<ZipArchiveEntry>() { // from class: org.apache.commons.compress.archivers.zip.ZipFile.2
            ...
            public int compare(ZipArchiveEntry zipArchiveEntry, ZipArchiveEntry zipArchiveEntry2) {
                ...
                long j2 = entry.getOffsetEntry().headerOffset - entry2.getOffsetEntry().headerOffset;
                if (j2 == 0) {
                    return 0;
                }
                return j2 < 0 ? -1 : 1;
    ...
    public class BoundedInputStream extends InputStream {
        ...
        public int read(byte[] bArr, int i2, int i3) throws IOException {
            ...
            synchronized (ZipFile.this.archive) {
                ...
                i4 = ZipFile.this.archive.read(bArr, i2, i3);
            }
            if (i4 > 0) {
                long j3 = i4;
                this.loc += j3;
