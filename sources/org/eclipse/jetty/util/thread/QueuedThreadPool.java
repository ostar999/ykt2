...
public class QueuedThreadPool extends AbstractLifeCycle implements ThreadPool.SizedThreadPool, Executor, Dumpable {
    ...
    private boolean startThread(int i2) {
    ...
    public boolean dispatch(Runnable runnable) {
        ...
        if (isRunning()) {
            ...
            if (this._jobs.offer(runnable)) {
                if ((idleThreads == 0 || size > idleThreads) && (i2 = this._threadsStarted.get()) < this._maxThreads) {
                    startThread(i2);
                }
                return true;
            }
    ...
    public String dumpThread(long j2) {
        Iterator<Thread> it = this._threads.iterator();
        while (it.hasNext()) {
            Thread next = it.next();
            if (next.getId() == j2) {
    ...
    public boolean interruptThread(long j2) {
        Iterator<Thread> it = this._threads.iterator();
        while (it.hasNext()) {
            Thread next = it.next();
            if (next.getId() == j2) {
    ...
    public void setMaxQueued(int i2) {
        if (isRunning()) {
    ...
    public void setMaxStopTimeMs(int i2) {
        this._maxStopTime = i2;
    }

    @Override // org.eclipse.jetty.util.thread.ThreadPool.SizedThreadPool
    public void setMaxThreads(int i2) {
        this._maxThreads = i2;
        if (this._minThreads > i2) {
            this._minThreads = i2;
        }
    }
