...
class JettyAwareLogger implements org.slf4j.Logger {
    ...
    private void log(Marker marker, int i2, String str, Object[] objArr, Throwable th) {
        if (objArr == null) {
            this._logger.log(marker, FQCN, i2, str, null, th);
            return;
        }
        if ((this._logger.isTraceEnabled() ? 0 : this._logger.isDebugEnabled() ? 10 : this._logger.isInfoEnabled() ? 20 : this._logger.isWarnEnabled() ? 30 : 40) <= i2) {
            this._logger.log(marker, FQCN, i2, MessageFormatter.arrayFormat(str, objArr).getMessage(), null, th);
        }
    }
